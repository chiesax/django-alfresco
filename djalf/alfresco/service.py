...
from djalf.alfresco import settings
from djalf.alfresco.cache import image_cache
from djalf.alfresco.thumbnail import parse_html_for_images
from djalf.alfresco import utils
...
class WebScript(object):
    ...
    def deserializer(self, **kwargs):
        ...
        while (1):            
            ...
            except (sax.SAXParseException, serializers.base.DeserializationError) as e:
...
class SearchWebScript(WebScript):
    ...
    def __init__(self, package='django', method='sortedsearch', format='xml'):
        super(SearchWebScript, self).__init__(package, method, format)
        self.url = None
        self.response = None
        self.params = {}
    ...
    def paginate(self, *args, **kwargs):
        ...
        if ('page' not in kwargs) and ('page_size' not in kwargs):
...
def generic_search(q, sort_by, limit, alf_ticket, cache_results=False, limit_range=False):
    ...
    KEY = None
    if cache_results:
        from djalf.alfresco.utils import generate_hex_key
...
class SpaceStore(object):
    ...
    def _build_url(self):
        if not self.path:
            from djalf.alfresco.models import Content
