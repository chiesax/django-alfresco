...
Category = apps.get_model('hierarchies', 'category')
Hierarchy = apps.get_model('hierarchies', 'hierarchy')
...
def get_hierarchies(parser, token):
    ...
    if bits[1] != 'as':
        raise template.TemplateSyntaxError(
            "%s tag requires the second arguement to be 'as'" % token.contents.split()[0])
...
def get_top_categories(parser, token):
    ...
    if bits[2] != 'as':
        raise template.TemplateSyntaxError(
            "%s tag requires the the third argument to be 'as'" % token.contents.split()[0])
